...
if(isset($_POST["valider"])) {
    if (!empty($_POST)){ 
        $target_file = $target_dir . basename($_FILES["photoJ"]["name"]);
        $uploadOk = 1;
        $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
        $check = getimagesize($_FILES["photoJ"]["tmp_name"]);
        $uploadOk = 1;
        if ($check !== false) {
            $uploadOk = 1;
        }
        ...
        if ($uploadOk == 0) {
            ...
            while($tab=$req3->fetch()){
                if ( !in_array($_POST['login'],$tab)){
                    ...
                    $score=0;
                    $statut="actif";
                    if (move_uploaded_file($_FILES["photoJ"]["tmp_name"], $target_file)) {
                        $file=basename( $_FILES["photoJ"]["name"]);  
                        $avatar=$file;
                        $bdd=connexion();
                        $req = $bdd->prepare("
